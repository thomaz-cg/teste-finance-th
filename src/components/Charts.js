...
import { CATEGORIES, CAT_MAP, fmt, getMonths, monthLabel, expensesByMonth, totalByCategory, currentYM, fixedAsExpenses } from '../helpers';
import { PieChart, Pie, Cell, Tooltip, ResponsiveContainer, BarChart, Bar, XAxis, YAxis, CartesianGrid } from 'recharts';

export default function Charts({ expenses, fixedList }) {
  const months = getMonths(expenses);
  const ym = currentYM();
  const allMonths = Array.from(new Set([ym, ...months])).sort().reverse();
  const [selMonth, setSelMonth] = useState(allMonths[0] || ym);
  ...
  const pieData   = CATEGORIES.filter(c => catTotals[c.id] > 0)
  ...
  const TooltipPie = ({ active, payload }) => {
    ...
    const total = monthExp.reduce((s,e)=>s+e.val,0);
    return (
  ...
  return (
    <div>
      <div style={{ marginBottom:28 }}>
        ...
        <p style={{ fontSize:14, color:'var(--gray-mid)', marginTop:4 }}>Análise visual incluindo fixos e variáveis</p>
      </div>
        ...
        : <>
          <div className="card" style={{ padding:24, marginBottom:20 }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'6px 18px', marginBottom:14 }}>
              {pieData.map(d=>(
                <span key={d.name} style={{ display:'flex', alignItems:'center', gap:5, fontSize:12, color:'var(--gray-mid)' }}>
          ...
          <div className="card" style={{ padding:24 }}>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'6px 18px', marginBottom:14 }}>
              {respData.map((d,i)=>(
                <span key={d.name} style={{ display:'flex', alignItems:'center', gap:5, fontSize:12, color:'var(--gray-mid)' }}>
                  <span style={{ width:10, height:10, borderRadius:2, background:respColors[i], flexShrink:0 }}></span>
                  {d.name} — {fmt(d.value)}
                </span>
              ))}
            </div>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={respData} barCategoryGap="40%">
                ...
                <Tooltip content={<TooltipBar/>} cursor={{ fill:'rgba(24,95,165,0.06)' }}/>
