...
import { CATEGORIES, RESPONSAVEIS, CAT_MAP, fmt, today, getMonths, monthLabel, currentYM, fixedAsExpenses } from '../helpers';
import { Plus, Trash2, RefreshCw } from 'lucide-react';

export default function Entries({ expenses, fixedList, onAdd, onDelete }) {
  ...
  // merge variable + fixed for display
  const activeFixed = fixedAsExpenses(fixedList);
  const allEntries  = [
    ...expenses,
    ...(showFixed ? activeFixed.map(f => ({ ...f, date: ym + '-' + String(fixedList.find(fl=>'fixed_'+fl.id===f.id)?.dueDay||1).padStart(2,'0') })) : []),
  ];
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:10, marginBottom:14, flexWrap:'wrap', alignItems:'center' }}>
        ...
        <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:13, color:'var(--gray-mid)', cursor:'pointer', marginLeft:4 }}>
