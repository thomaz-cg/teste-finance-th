import React, { useState } from 'react';
import { CATEGORIES, RESPONSAVEIS, CAT_MAP, fmt } from '../helpers';
import { Plus, Trash2, RefreshCw, Calendar } from 'lucide-react';

export default function Fixed({ fixedList, onAdd, onDelete, onToggle, onUpdate }) {
  const [form, setForm] = useState({
    desc: '', val: '', cat: 'Assinaturas', resp: 'Casal', dueDay: '10', obs: '',
  });
  const [saving, setSaving] = useState(false);

  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));

  const handleAdd = async () => {
    if (!form.desc.trim() || !form.val || parseFloat(form.val) <= 0) {
      alert('Preencha a descrição e um valor válido.');
      return;
    }
    const day = parseInt(form.dueDay);
    if (!day || day < 1 || day > 31) {
      alert('Informe um dia de vencimento válido (1 a 31).');
      return;
    }
    setSaving(true);
    await onAdd({
      desc:   form.desc.trim(),
      val:    parseFloat(form.val),
      cat:    form.cat,
      resp:   form.resp,
      dueDay: day,
      obs:    form.obs.trim(),
      active: true,
    });
    setForm({ desc: '', val: '', cat: 'Assinaturas', resp: 'Casal', dueDay: '10', obs: '' });
    setSaving(false);
  };

  const activeList   = fixedList.filter(f => f.active);
  const inactiveList = fixedList.filter(f => !f.active);
  const totalFixed   = activeList.reduce((s, f) => s + f.val, 0);

  const ordinal = (d) => `Todo dia ${d}`;

  return (
    <div>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700 }}>Gastos Fixos</h1>
        <p style={{ fontSize: 14, color: 'var(--gray-mid)', marginTop: 4 }}>
          Cadastre uma vez — entram automaticamente no resumo todo mês
        </p>
      </div>

      {/* Summary */}
      <div className="metric-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(160px,1fr))', marginBottom: 20 }}>
        <div className="metric-card">
          <div className="metric-label"><RefreshCw size={12}/> Total fixo/mês</div>
          <div className="metric-value purple">{fmt(totalFixed)}</div>
          <div className="metric-sub">{activeList.length} gasto{activeList.length !== 1 ? 's' : ''} ativo{activeList.length !== 1 ? 's' : ''}</div>
        </div>
        <div className="metric-card">
          <div className="metric-label"><Calendar size={12}/> Próximo vencimento</div>
          <div className="metric-value blue" style={{ fontSize: 18 }}>
            {activeList.length > 0
              ? (() => {
                  const hoje = new Date().getDate();
                  const proximos = activeList
                    .map(f => ({ ...f, diff: f.dueDay >= hoje ? f.dueDay - hoje : 31 - hoje + f.dueDay }))
                    .sort((a, b) => a.diff - b.diff);
                  return proximos[0] ? `Dia ${proximos[0].dueDay} — ${proximos[0].desc}` : '—';
                })()
              : '—'}
          </div>
          <div className="metric-sub">mais próximo</div>
        </div>
      </div>

      {/* Form */}
      <div className="form-card">
        <div className="form-section-title">+ Novo gasto fixo</div>
        <div className="form-grid">
          <div className="form-group">
            <label>Descrição</label>
            <input type="text" placeholder="Ex: Netflix, Internet, Celular"
              value={form.desc} onChange={e => set('desc', e.target.value)}
              onKeyDown={e => e.key === 'Enter' && handleAdd()} />
          </div>
          <div className="form-group">
            <label>Valor (R$)</label>
            <input type="number" placeholder="0,00" min="0" step="0.01"
              value={form.val} onChange={e => set('val', e.target.value)} />
          </div>
          <div className="form-group">
            <label>Dia do vencimento</label>
            <input type="number" placeholder="Ex: 10" min="1" max="31"
              value={form.dueDay} onChange={e => set('dueDay', e.target.value)} />
          </div>
        </div>
        <div className="form-grid-2">
          <div className="form-group">
            <label>Categoria</label>
            <select value={form.cat} onChange={e => set('cat', e.target.value)}>
              {CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.icon} {c.id}</option>)}
            </select>
          </div>
          <div className="form-group">
            <label>Responsável</label>
            <select value={form.resp} onChange={e => set('resp', e.target.value)}>
              {RESPONSAVEIS.map(r => <option key={r} value={r}>{r}</option>)}
            </select>
          </div>
        </div>
        <div className="form-group" style={{ marginBottom: 14 }}>
          <label>Observações (opcional)</label>
          <input type="text" placeholder="Ex: plano família, parcela 3/12"
            value={form.obs} onChange={e => set('obs', e.target.value)} />
        </div>
        <button className="btn-primary" onClick={handleAdd} disabled={saving}>
          <Plus size={16} /> {saving ? 'Salvando...' : 'Adicionar gasto fixo'}
        </button>
      </div>

      {/* Active list */}
      {activeList.length > 0 && (
        <div className="card" style={{ marginBottom: 16 }}>
          <div style={{ padding: '16px 20px 0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div className="section-title" style={{ marginBottom: 0 }}>✅ Ativos</div>
            <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--purple-dark)' }}>{fmt(totalFixed)}/mês</span>
          </div>
          <div style={{ marginTop: 8 }}>
            {activeList
              .sort((a, b) => a.dueDay - b.dueDay)
              .map(f => {
              const cat = CAT_MAP[f.cat] || {};
              return (
                <div key={f.id} className="expense-item">
                  <div className="expense-icon" style={{ background: cat.bg || '#f5f5f5' }}>{cat.icon || '📦'}</div>
                  <div className="expense-info">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <span className="expense-desc">{f.desc}</span>
                      <span className="due-badge"><Calendar size={10}/> {ordinal(f.dueDay)}</span>
                      <span className="fixed-badge"><RefreshCw size={9}/> Fixo</span>
                    </div>
                    <div className="expense-meta">{f.cat} · {f.resp}{f.obs ? ` · ${f.obs}` : ''}</div>
                  </div>
                  <div className="expense-amount">{fmt(f.val)}</div>
                  <label className="toggle" title="Desativar temporariamente">
                    <input type="checkbox" checked={f.active} onChange={e => onToggle(f.id, e.target.checked)} />
                    <span className="toggle-slider"></span>
                  </label>
                  <button className="btn-icon" onClick={() => { if (window.confirm(`Remover "${f.desc}"?`)) onDelete(f.id); }}>
                    <Trash2 size={15} />
                  </button>
                </div>
              );
            })}
          </div>
        </div>
      )}

      {/* Inactive list */}
      {inactiveList.length > 0 && (
        <div className="card">
          <div style={{ padding: '16px 20px 0' }}>
            <div className="section-title" style={{ marginBottom: 0, color: 'var(--gray-mid)' }}>⏸ Inativos (não entram no mês)</div>
          </div>
          <div style={{ marginTop: 8 }}>
            {inactiveList.map(f => {
              const cat = CAT_MAP[f.cat] || {};
              return (
                <div key={f.id} className="expense-item" style={{ opacity: 0.5 }}>
                  <div className="expense-icon" style={{ background: cat.bg || '#f5f5f5' }}>{cat.icon || '📦'}</div>
                  <div className="expense-info">
                    <div className="expense-desc">{f.desc}</div>
                    <div className="expense-meta">{f.cat} · dia {f.dueDay}{f.obs ? ` · ${f.obs}` : ''}</div>
                  </div>
                  <div className="expense-amount">{fmt(f.val)}</div>
                  <label className="toggle" title="Reativar">
                    <input type="checkbox" checked={f.active} onChange={e => onToggle(f.id, e.target.checked)} />
                    <span className="toggle-slider"></span>
                  </label>
                  <button className="btn-icon" onClick={() => { if (window.confirm(`Remover "${f.desc}"?`)) onDelete(f.id); }}>
                    <Trash2 size={15} />
                  </button>
                </div>
              );
            })}
          </div>
        </div>
      )}

      {fixedList.length === 0 && (
        <div className="card">
          <div className="empty-state">
            <div className="empty-state-icon">🔄</div>
            Nenhum gasto fixo cadastrado ainda.
          </div>
        </div>
      )}
    </div>
  );
}
