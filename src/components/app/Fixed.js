...
import { CATEGORIES, CAT_MAP, fmt, getResponsaveis } from '../../helpers';
import { Plus, Trash2, RefreshCw, Calendar } from 'lucide-react';

export function Fixed({ fixedList, profile, onAdd, onDelete, onToggle }) {
  const [form, setForm] = useState({ desc:'', val:'', cat:'Assinaturas', resp:'Casal', dueDay:'10', obs:'' });
  ...
  const handleAdd = async () => {
    ...
    if (!day||day<1||day>31) { alert('Dia inválido (1 a 31).'); return; }
    setSaving(true);
    await onAdd({ desc:form.desc.trim(), val:parseFloat(form.val), cat:form.cat, resp:form.resp, dueDay:day, obs:form.obs.trim(), active:true });
    ...
    <div>
      ...
      <div className="metric-grid" style={{ gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', marginBottom:20 }}>
        ...
        <div className="metric-card">
          ...
          <div className="metric-value blue" style={{ fontSize:16 }}>
            {activeList.length>0
              ? (() => {
                  const hoje=new Date().getDate();
                  const prox=activeList.map(f=>({...f,diff:f.dueDay>=hoje?f.dueDay-hoje:31-hoje+f.dueDay})).sort((a,b)=>a.diff-b.diff);
                  return prox[0]?`Dia ${prox[0].dueDay} — ${prox[0].desc}`:'—';
                })()
              :'—'}
          </div>
      ...
      <div className="form-card">
        ...
        <div className="form-grid">
          <div className="form-group">
            ...
            <input type="text" placeholder="Ex: Netflix, Internet, Celular" value={form.desc} onChange={e=>set('desc',e.target.value)}/>
          </div>
          <div className="form-group">
            <label>Valor (R$)</label>
