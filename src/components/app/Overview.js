...
export default function Overview({ expenses, fixedList, budget, profile, onNavigate }) {
  ...
  const chartData = last6.map(m => ({
    name: monthLabel(m),
    Variáveis: expensesByMonth(expenses,m).reduce((s,e)=>s+e.val,0),
    Fixos: activeFixed.reduce((s,f)=>s+f.val,0),
  }));
  ...
  return (
    <div>
      <div style={{ marginBottom:24 }}>
        ...
        <p style={{ fontSize:14, color:'var(--gray-mid)', marginTop:4 }}>Resumo de {monthLabel(ym)} — fixos + variáveis</p>
      </div>
      ...
      <div className="metric-grid">
        ...
        <div className="metric-card">
          ...
          <div className="metric-sub">{fixedList.filter(f=>f.active).length} ativos</div>
        </div>
        <div className="metric-card">
          <div className="metric-label"><TrendingUp size={12}/> Variáveis</div>
