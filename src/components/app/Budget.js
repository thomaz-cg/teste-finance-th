...
export default function Budget({ expenses, fixedList, budget, setBudget }) {
  ...
  const tl=budget.total, ts=getStatus(totalSpent,tl);
  const tp=tl>0?Math.min(100,Math.round(totalSpent/tl*100)):0;
  const tc=tp>100?'#A32D2D':tp>70?'#854F0B':'#185FA5';

  return (
    <div>
      <div style={{ marginBottom:28 }}>
        <h1 style={{ fontSize:22, fontWeight:700 }}>Orçamento</h1>
        <p style={{ fontSize:14, color:'var(--gray-mid)', marginTop:4 }}>Limites mensais — fixos + variáveis</p>
      </div>
      ...
      {totalFixed>0&&(
        <div style={{ background:'var(--purple-light)', border:'1px solid rgba(83,52,137,0.15)', borderRadius:'var(--radius-md)', padding:'14px 18px', marginBottom:20, display:'flex', alignItems:'center', gap:12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:12, color:'var(--purple-dark)', opacity:0.8, marginTop:2 }}>Já incluídos no cálculo abaixo</div>
          </div>
        </div>
      )}
      ...
      <div className="card" style={{ padding:24 }}>
          ...
          <div className="budget-bar" style={{ paddingBottom:18, marginBottom:18, borderBottom:'1px solid rgba(0,0,0,0.06)' }}>
            <div className="budget-bar-header">
              ...
              <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                <div className="budget-bar-amounts">{fmt(totalSpent)} de {fmt(tl)}</div>
                {ts&&<span style={{ fontSize:11, padding:'2px 8px', borderRadius:20, background:ts.bg, color:ts.color, fontWeight:700 }}>{ts.label}</span>}
              </div>
            </div>
            <div className="budget-bar-track"><div className="budget-bar-fill" style={{ width:`${tp}%`, background:tc }}/></div>
          </div>
        )}

        {CATEGORIES.map(cat=>{
          const spent=catTotals[cat.id]||0, limit=budget.cats[cat.id]||0;
        ...
        {tl===0&&Object.values(budget.cats).every(v=>!v)&&<div className="empty-state"><div className="empty-state-icon">🎯</div>Configure seus limites acima.</div>}
      </div>
    </div>
  );
}
